...
function parseNativeVersion(fileContent) {
  const matches = [
    ...fileContent.matchAll(/export\s+const\s+NATIVE_VERSION\s*=\s*['"]([^'"]+)['"]/g),
  ];
  if (matches.length !== 1) {
...
function crossCheckMetadata(
  ...
  if (gradleVersionName !== nativeVersion) {
    throw new Error(
      `NATIVE_VERSION (${nativeVersion}) differs from build.gradle versionName (${gradleVersionName})!`
    );
  }
  ...
  if (prevVersionCode && gradleVersionCode <= prevVersionCode) {
    throw new Error(
      `versionCode (${gradleVersionCode}) is not greater than the previous release (${prevVersionCode})!`
    );
  }
  return true;
}
...
// --- Test A ---
console.log(
  'Running Test A: Correct parsing of NATIVE_VERSION when dynamic APP_VERSION is present...'
);
const testASrc = `
...
try {
  ...
  assert.match(
    e.message,
    /Unable to resolve NATIVE_VERSION/,
    'Test B Failed: Incorrect error message'
  );
...
try {
  ...
  assert.match(
    e.message,
    /differs from build.gradle versionName/,
    'Test C Failed: Incorrect error message'
  );
