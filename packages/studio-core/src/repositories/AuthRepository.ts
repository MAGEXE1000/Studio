...
import { syncProfileListener } from '../lib/permissions';
import { logActivity } from '../lib/diagnostics/activityLogger';
...
export class AuthRepository {
  ...
  private toAuthUser(u: User | null): AuthUser | null {
    if (!u) return null;
    return {
      uid: u.uid,
      email: u.email,
  ...
  private lastAuthUser: AuthUser | null = null;
  private readonly authCallbacks = new Set<(u: AuthUser | null) => void>();
  
  public subscribeAuth(cb: (u: AuthUser | null) => void): () => void {
    ...
    if (this.lastAuthUser !== null) {
      cb(this.lastAuthUser);
    }
  
    const unsub = onAuthStateChanged(auth, (u) => {
      const authUser = this.toAuthUser(u);
  ...
  public async registerEmail(email: string, password: string, displayName?: string): Promise<void> {
    const auth = getFirebaseAuth();
    if (!auth) throw new Error('Firebase not configured');
  ...
  public async updateDisplayName(name: string): Promise<void> {
    ...
    if (auth.currentUser) {
      const authUser = this.toAuthUser(auth.currentUser);
      this.lastAuthUser = authUser;
  ...
  public updateLocalAuthUser(details: Partial<AuthUser>) {
    if (this.lastAuthUser) {
      this.lastAuthUser = { ...this.lastAuthUser, ...details };
      for (const cb of this.authCallbacks) {
        cb(this.lastAuthUser);
      }
    }
  ...
  public getSignInProviders(): string[] {
    const auth = getFirebaseAuth();
