...
1. **Auth Session Persistence**: Bumping versions or updating the app must not log users out. The active Supabase/Firebase session token must persist.
...
2. **Download & Checksums**: Download the APK from the published release tag and verify that its SHA-256 hash matches the value declared in `app-release.json`.
