...
const searchDirs = [path.join(pkgRoot, 'src'), path.join(pkgRoot, 'public')];
...
function scanFile(filePath) {
  ...
  const staticSpanRegex =
    /<span\s+[^>]*class(?:Name)?=["'][^"']*material-symbols-outlined[^"']*["'][^>]*>\s*([a-z0-9_]{3,30})\s*<\/span>/gi;
  let match;
  ...
  const dynamicSpanRegex =
    /<span\s+[^>]*class(?:Name)?=["'][^"']*material-symbols-outlined[^"']*["'][^>]*>\s*{([\s\S]*?)}\s*<\/span>/gi;
  ...
  const staticIRegex =
    /<i\s+[^>]*class(?:Name)?=["'][^"']*material-symbols-outlined[^"']*["'][^>]*>\s*([a-z0-9_]{3,30})\s*<\/i>/gi;
  ...
  const dynamicIRegex =
    /<i\s+[^>]*class(?:Name)?=["'][^"']*material-symbols-outlined[^"']*["'][^>]*>\s*{([\s\S]*?)}\s*<\/i>/gi;
...
const textParam = encodeURIComponent(
  validIcons.join(' ') + ' abcdefghijklmnopqrstuvwxyz_0123456789'
);
...
async function run() {
  try {
    ...
    console.log(
      `download-subset: ✓ Font optimized: ${(sizeBefore / 1024 / 1024).toFixed(2)} MB → ${(sizeAfter / 1024).toFixed(1)} KB`
    );
  } catch (err) {
    console.warn(
      'download-subset: ⚠ Failed to optimize font (continuing with existing font for offline build):',
      err.message || err
    );
    process.exit(0);
  }
