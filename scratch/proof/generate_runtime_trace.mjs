...
const mockAppInstaller = {
  installApkDirect: async (call) => {
    addTrace(
      'Native (Java)',
      'AppInstallerPlugin.installApkDirect',
      'ENTER',
      `filePath=${call.filePath}`
    );
    addTrace(
      'Native (Java)',
      'AppInstallerPlugin.triggerInstallation',
      'STEP',
      'Session.create() success. sessionId=999'
    );
    ...
    addTrace(
      ...
      'AppInstallerPlugin.triggerInstallation',
      'STEP',
      'Session.commit() start. Calling session.commit()'
    );
    addTrace(
      'Native (Java)',
      'AppInstallerPlugin.triggerInstallation',
      'STEP',
      'Session.commit() finished and session closed'
    );
    ...
    setTimeout(async () => {
      const { simulateAppStateChange } = await import(
        `file://${path.join(repoRoot, 'packages/studio-core/dist/src/lib/startup/startupCoordinator.js').replace(/\\/g, '/')}`
      );
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(async () => {
        ...
        addTrace(
          ...
          'ActivityManager',
          'LIFECYCLE',
          'onActivityResumed (Studio Activity)'
        );
...
globalThis.fetch = async () => ({
  ok: true,
  json: async () => ({ version: '4.0.25', versionCode: 40025, apkUrl: 'https://cdn' }),
});

async function run() {
  const otaModuleUrl = `file://${path.join(repoRoot, 'packages/studio-core/dist/src/lib/otaUpdate.js').replace(/\\/g, '/')}`;
  const { checkForUpdate, downloadUpdate, applyUpdate, UpdaterFlightRecorder } = await import(
    otaModuleUrl
  );
