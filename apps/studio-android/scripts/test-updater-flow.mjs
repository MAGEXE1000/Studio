...
const reportPath =
  'C:/Users/ayuda/.gemini/antigravity/brain/80b974db-03ec-4bf1-86df-33510e63d39d/scratch/updater_self_test_report.md';
...
async function runSelfTest() {
  ...
  try {
    ...
    log(
      'Fetch Metadata',
      'SUCCESS',
      `Successfully fetched version metadata for v${metadata.version} (Code ${metadata.versionCode})`
    );
    ...
    log(
      'Download APK',
      'SUCCESS',
      `APK successfully downloaded to temporary file (${(buffer.byteLength / (1024 * 1024)).toFixed(2)} MB)`
    );
    ...
    try {
      ...
      if (calculatedHash !== metadata.sha256) {
        throw new Error(
          `Checksum mismatch! Metadata: ${metadata.sha256}, Calculated: ${calculatedHash}`
        );
      }
      log(
        'SHA-256 Verification',
        'SUCCESS',
        `SHA-256 matches production metadata: ${calculatedHash}`
      );
      ...
      const signInfoVerbose = execSync(
        `${apksigner} verify --verbose --print-certs "${tempApkPath}"`,
        { encoding: 'utf8' }
      );
      ...
      log(
        ...
        'SUCCESS',
        `Signed with modern scheme. V2=${v2Scheme}, V3=${v3Scheme}`
      );

      // 6. Keystore details check
      log('Keystore Inspection', 'INFO', 'Parsing Owner, Issuer, and Validity via keytool...');
      const keytoolOut = execSync(`keytool -printcert -jarfile "${tempApkPath}"`, {
        encoding: 'utf8',
      });
      ...
      if (now < validFrom || now > validUntil) {
        throw new Error(
          `Certificate is expired or not yet valid! validity: ${validFromStr} to ${validUntilStr}`
        );
      }
      log(
        'Keystore Inspection',
      ...
      if (currentSignature !== metadata.signatures) {
        throw new Error(
          `Certificate fingerprint mismatch! Expected: ${metadata.signatures}, Found: ${currentSignature}`
        );
      }
      log(
        'Keystore Fingerprint',
        'SUCCESS',
        `Certificate fingerprint matches production signature metadata: ${currentSignature}`
      );
  ...
  for (const r of results) {
    ...
    reportLines.push(
      `| ${r.stage} | ${icon} **${r.status}** | ${r.message.replace(/\n/g, '<br>')} |`
    );
  }
