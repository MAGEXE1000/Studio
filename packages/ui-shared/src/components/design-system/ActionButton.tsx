...
export interface ActionButtonProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, 'onClick' | 'onDrag' | 'onDragStart' | 'onDragEnd' | 'onDragOver' | 'onAnimationStart'> {
...
export const ActionButton: React.FC<ActionButtonProps> = ({
  ...
  const getButtonStyles = () => {
    ...
    let shadow = '0 2px 8px rgba(0,0,0,0.15)';
  ...
  return (
    <motion.button
      ...
      whileHover={isButtonDisabled ? {} : { scale: 1.05, boxShadow: '0 6px 16px rgba(0,0,0,0.25)', y: -1 }}
    ...
    >
      ...
      {children && (
        <span style={{ marginLeft: '8px', lineHeight: 1 }}>{children}</span>
      )}
    </motion.button>
  );
};
