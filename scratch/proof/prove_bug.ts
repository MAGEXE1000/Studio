import {
  ...
  updateGlobalState,
  stateListeners,
} from '../../packages/studio-core/src/lib/updater/stateMachine';
...
async function runProof() {
  ...
  stateListeners.add((state) => {
    console.log(
      `[STATE CHANGE] updateState: ${state.updateState} | updateAvailable: ${state.updateAvailable}`
    );
  });
  ...
  const { safeTransition } =
    await import('../../packages/studio-core/src/lib/updater/stateMachine');

  const result = safeTransition(
    'COMPARE_VERSION',
    'UPDATE_AVAILABLE',
    'Simulated background check complete'
  );
