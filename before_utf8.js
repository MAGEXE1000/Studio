...
const state = {
  elements: [],
  connections: [],
  // ΓöÇΓöÇ Scenes (v3.0.63+) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  // Up to 3 scenes per project. Each scene captures a stage plot
  // (its own elements, connections, and nextId). Other data
  // (members, gear, setlist, riderΓÇª) is shared across scenes.
  ...
  riderNeeds: [
    ...
    { id:'rn3', type:'power',   value:'2├ù 20A circuits, distro Stage Left' },
  ],
  _rnNextId: 4,
};

// ΓöÇΓöÇ Cable / Power constants ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const NEED_TYPES = {
  foh:      { label:'FOH Protocol',          labelEs:'Protocolo FOH',           color:'#7aafff',
    ...
    presetsEs: ['Dante Primario/Secundario @ 96kHz','Consola anal├│gica, m├¡n. 32 canales','Consola digital, m├¡n. 48 canales','H├¡brido MADI + Dante','Consola a cargo de la banda','Sin preferencia'] },
  monitor:  { label:'Monitor / IEM',         labelEs:'Monitor / IEM',           color:'#ff7439',
    ...
    presetsEs: ['M├¡nimo 4 mezclas IEM est├⌐reo','6 mezclas de monitores cu├▒a','2 IEM est├⌐reo + 4 cu├▒as','8 mezclas aux independientes','IEM a cargo de la banda, env├¡os por el venue','Sin monitores requeridos'] },
  power:    { label:'Power Requirement',     labelEs:'Requerimiento El├⌐ctrico', color:'#c5ffc9',
    presets:   ['2├ù 20A circuits Stage Left','3├ù 20A circuits Stage Center','4├ù 15A circuits','Distro provided by band','Standard venue power sufficient'],
    presetsEs: ['2├ù circuitos 20A Lateral Izquierdo','3├ù circuitos 20A Centro del escenario','4├ù circuitos 15A','Distribuci├│n el├⌐ctrica a cargo de la banda','Alimentaci├│n est├índar del venue suficiente'] },
  backline: { label:'Backline',              labelEs:'Backline',                color:'#ffd700',
    ...
    presetsEs: ['Todo el backline a cargo de la banda','Bater├¡a requerida (el venue la proporciona)','Amplificador de guitarra requerido (el venue)','Amplificador de bajo requerido (el venue)','Backline completo requerido del venue'] },
  wireless: { label:'Wireless Coordination', labelEs:'Coordinaci├│n Inal├ímbrica',color:'#c8a2ff',
  ...
  crew:     { label:'Stage Crew',            labelEs:'Crew de Escenario',       color:'#ff8dd4',
    ...
    presetsEs: ['M├¡nimo 2 crew en carga','1 ingeniero de monitores dedicado requerido','Ingeniero FOH viaja con la banda','Crew a cargo de la banda','Stage manager a cargo del venue'] },
...
const DEFAULT_NOTES_ES = 'El artista proporciona todos los instrumentos, transmisores IEM y rack de reproducci├│n. El venue debe proporcionar todos los micr├│fonos, soportes y cableado XLR seg├║n la lista de canales. El sistema PA debe ser capaz de 105dB continuo en FOH sin distorsi├│n. Los front-fills son obligatorios para las primeras 3 filas. Todos los sistemas inal├ímbricos deben coordinarse en frecuencia antes del montaje.';
...
const TRANSLATIONS = {
  en: {
    ...
    noGear:'No gear items added ΓÇö visit the Gear tab to build your list.',
    ...
    noConns:'No connections defined ΓÇö use Connect mode in the Editor to link elements.',
    noElems:'No elements placed ΓÇö connectivity summary will appear here.',
    ...
    staleWarning:'ΓÜá Plot may be outdated ΓÇö last updated more than 30 days ago',
    ...
    pdfSaved:'PDF saved successfully', pdfFailed:'PDF export failed ΓÇö try again',
    ...
    // Library ΓÇö Drums
    libDrumKit:'Drum Kit', libEDrums:'E-Drums', libPercussion:'Percussion', libCajon:'Caj├│n',
    // Library ΓÇö Instruments
    ...
    slMainSet:'Main Set', slMainDesc:'The core journey ΓÇö 3ΓÇô4 song clusters alternating high & low energy in a "W" shape.',
    slClosingSegment:'Closing Segment', slClosingDesc:'A powerful crowd-pleaser to end the main set on a high note.',
    slEncore:'Encore', slEncoreDesc:'An optional final song or two ΓÇö often a major hit after the main set concludes.',
    ...
    batchPasteHint:'Paste your song list below ΓÇö one title per line. Or upload a .txt or .pdf file. Smart parsing handles numbered lists, bullets, and BPM / key hints.',
  ...
  es: {
    // PDF / Export
    coverLabel:'Rider T├⌐cnico',
    docId:'ID del documento', expDate:'Fecha de exportaci├│n', elements:'Elementos',
    stagePlot:'Diagrama de escenario', inputList:'Lista de canales', bandMembers:'Integrantes',
    connectivity:'Conectividad', riderReqs:'Requerimientos t├⌐cnicos',
    setlist:'Setlist', lightingRider:'Rider de iluminaci├│n',
    techNotes:'Notas t├⌐cnicas', gear:'Equipo / Carga',
    colCh:'CH#', colInstrument:'Instrumento', colPerformer:'M├║sico',
    colMicDI:'Mic / DI', colSource:'Fuente', colNotes:'Notas',
    colItem:'Art├¡culo', colCategory:'Categor├¡a', colQty:'Cant.',
    inputEmpty:'Agrega elementos en el Editor para llenar esta lista.',
    noGear:'Sin equipo ΓÇö ve a la pesta├▒a Equipo para armar tu lista.',
    noAssignments:'Sin asignaciones',
    signalPatch:'Parche de se├▒al', signalSummary:'Resumen de se├▒al',
    phantomCh:'Canales +48V', signalPaths:'Rutas de se├▒al', totalEl:'Total de elementos',
    activeInputs:'Entradas activas', outputRoutes:'Salidas',
    noConns:'Sin conexiones ΓÇö usa Conectar en el Editor para enlazar elementos.',
    noElems:'Sin elementos ΓÇö el resumen aparecer├í aqu├¡ al agregarlos.',
    scaleLabel:'Escala: 1:50', clickEdit:'Toca para editar',
    footerSub:'Editor profesional de diagrama de escenario y rider t├⌐cnico',
    reqs:'Requerimientos t├⌐cnicos',
    includeSections:'Incluir secciones', gearShort:'Equipo / Carga',
    verLabel:'Ver', lastUpdatedLabel:'├Ültima actualizaci├│n',
    staleWarning:'ΓÜá El diagrama puede estar desactualizado ΓÇö m├ís de 30 d├¡as sin modificarse',
    noElements:'Sin elementos en el escenario',
    placeholderLabel:'Marcador',
    lightingPlaceholderText:'El m├│dulo de rider de iluminaci├│n forma parte de la hoja de ruta de desarrollo. Adjunta un documento de especificaci├│n de iluminaci├│n por separado si es necesario.',
    mobExport:'Exportar', mobOptions:'Opciones',
    exportSettingsLabel:'Ajustes de exportaci├│n', formatLabel:'Formato', sectionsLabel:'Secciones',
    techNotesShort:'Notas t├⌐cnicas', lightingShort:'Iluminaci├│n', shareQR:'Compartir / QR',
    // UI ΓÇö views & actions
    ...
    pdfSaved:'PDF guardado', pdfFailed:'Error al exportar PDF ΓÇö intenta de nuevo',
    linkCopied:'Enlace copiado',
    maxMembers:'M├íximo 8 integrantes',
    ...
    loadPresetConfirm:'┬┐Cargar preset "{name}"? Reemplazar├í el escenario actual.',
    ...
    // Category bar
    catMics:'Micros', catDrums:'Bater├¡a', catInst:'Instrumentos', catAmps:'Amps', catAudio:'Audio', catUtil:'Utiler├¡a',
    // Accordion
    accMics:'Micr├│fonos', accDrums:'Bater├¡a', accInst:'Instrumentos', accAmps:'Amplificadores', accAudio:'Audio', accUtil:'Utiler├¡a',
    // Sidebar
    sbSectionLabel:'Elementos',
    // Toolbar
    tbGrid:'Grilla', tbSnap:'Ajuste', tbConnect:'Conectar', tbLines:'L├¡neas', tbClear:'Limpiar',
    // Status bar
    sbZoom:'Zoom', sbElements:'Elementos', sbLines:'L├¡neas', sbSel:'Sel', sbSystemReady:'Listo',
    autosaveOff:'AUTOGUARDADO: OFF', autosaveOn:'AUTOGUARDADO: ON',
    // Stage labels
    downstageLabel:'Frente / P├║blico', dragHere:'Arrastra elementos aqu├¡',
    // Properties panel
    propLabel:'Etiqueta', propPerformer:'M├║sico', propChannel:'Canal',
    propRotation:'Rotaci├│n', propScale:'Escala', propInputSrc:'Fuente de entrada',
    propOutput:'Salida', propRoles:'Roles', propNotes:'Notas',
    // Settings
    settingsTitle:'Ajustes',
    settingsLang:'Idioma', settingsLangHint:'Cambia el idioma de la app.',
    settingsNavOrder:'Orden de navegaci├│n', settingsNavOrderHint:'Arrastra para reordenar las pesta├▒as.',
    settingsCanvasBg:'Fondo del lienzo', settingsCanvasBgHint:'Color de fondo del escenario.',
    settingsGridSize:'Tama├▒o de grilla', settingsGridSizeHint:'Espaciado de las l├¡neas de la grilla.',
    ...
    settingsConn:'Mostrar conexiones', settingsConnHint:'L├¡neas entre elementos conectados.',
    settingsCableLen:'Mostrar largo del cable', settingsCableLenHint:'Muestra la longitud aproximada de cada conexi├│n en metros.',
    settingsAutoWire:'Cableado autom├ítico', settingsAutoWireHint:'Conecta autom├íticamente elementos compatibles al colocarlos.',
    settingsTheme:'Tema', settingsThemeHint:'Color de acento de la interfaz.',
    themeElectric:'El├⌐ctrico', themeLime:'Lima', themeCyan:'Cian', themeAmber:'├ümbar', themeViolet:'Violeta', themeRose:'Rosa',
    // Library ΓÇö Mics
    libSM58:'SM58', libCondenser:'Condensador', libAmpMic:'Mic de instrumento',
    libWireless:'Inal├ímbrico', libBoundary:'PZM', libDrumClip:'Clip de bater├¡a', libMicStand:'Pedestal',
    // Library ΓÇö People
    libPerformer:'M├║sico', libVocalist:'Vocalista', libGuitarist:'Guitarrista',
    libBassist:'Bajista', libDrummer:'Baterista', libKeyboardist:'Tecladista', libSaxophonist:'Saxofonista', libTech:'T├⌐cnico',
    // Library ΓÇö Drums
    libDrumKit:'Bater├¡a', libEDrums:'Bater├¡a elec.', libPercussion:'Percusi├│n', libCajon:'Caj├│n',
    // Library ΓÇö Instruments
    libElecGuitar:'Guitarra elec.', libAcouGuitar:'Guitarra ac├║s.', libBassGuitar:'Bajo',
    ...
    libDrumFill:'Fill bater├¡a', libDrumSub:'Sub bater├¡a', libSideFill:'Side fill',
    libMainPAL:'PA principal I', libMainPAR:'PA principal D', libDelayTower:'Torre delay',
    libFrontFill:'Front fill', libHeadphoneAmp:'Amp aud├¡fonos',
    // Library ΓÇö Utilities
    ...
    libAmpRack:'Rack amp', libEffectsRack:'Rack efectos', libWirelessRack:'Rack inal├ím.',
    libLaptop:'Laptop', libIntercom:'Intercom', libOutlet:'Tomacorriente',
    setupTitle:'Configuraci├│n', setupSubtitle:'Documentos del show e info de la banda',
    setupRiderSub:'Lista de canales, entradas de mic y requerimientos t├⌐cnicos',
    ...
    backToSetup:'Configuraci├│n',
    slSongsCount:'Canciones', slTotalDuration:'Duraci├│n total', slAvgEnergy:'Energ├¡a prom.',
    slSegments:'Segmentos',
    slCurrentArrangement:'Arreglo\nactual', slAddNewTrack:'Agregar canci├│n',
    slSetlistInsights:'An├ílisis del setlist',
    ...
    slAddSection:'Agregar secci├│n',
    slOpeningSegment:'Segmento de apertura', slOpeningDesc:'Un inicio potente y de alta energ├¡a para captar la atenci├│n y establecer presencia.',
    slMainSet:'Set principal', slMainDesc:'El viaje central ΓÇö bloques de 3ΓÇô4 canciones alternando energ├¡a alta y baja en forma de "W".',
    slClosingSegment:'Segmento de cierre', slClosingDesc:'Un tema fuerte para terminar el set principal en alto.',
    slEncore:'Encore', slEncoreDesc:'Una o dos canciones opcionales al final ΓÇö generalmente un hit despu├⌐s de cerrar el set principal.',
    sngNewSong:'Nueva canci├│n', sngTitle:'T├¡tulo', sngArtist:'Artista', sngBpm:'BPM',
    sngKey:'Tonalidad', sngDuration:'Duraci├│n (MM:SS)', sngNotes:'Notas', sngCancel:'Cancelar',
    sngAddToSection:'Agregar a secci├│n',
    ...
    prefStageBalance:'Balance de escenario', prefStageBalanceHint:'Muestra la distribuci├│n de peso del escenario al colocar elementos.',
    prefElementLabels:'Etiquetas de elementos', prefElementLabelsHint:'Muestra categor├¡a y tipo debajo de cada elemento en el lienzo.',
    prefAnimations:'Animaciones', prefAnimationsHint:'Activa transiciones suaves y micro-animaciones en toda la interfaz.',
    prefSmartSuggestions:'Sugerencias inteligentes', prefSmartSuggestionsHint:'Sugiere ubicaciones ├│ptimas bas├índose en configuraciones de escenario comunes.',
    prefLastUpdated:'├Ültima actualizaci├│n', prefLastUpdatedHint:'Muestra cu├índo se modific├│ el proyecto por ├║ltima vez.',
    ...
    gearTotalItems:'Total', gearCategories:'Categor├¡as', gearChecked:'Revisados',
    ...
    batchPasteHint:'Pega tu lista de canciones abajo ΓÇö un t├¡tulo por l├¡nea. O sube un archivo .txt o .pdf. El parser detecta listas numeradas, vi├▒etas, y pistas de BPM / tonalidad.',
...
const library = {
  ...
  drums: [
    ...
    { name: 'Caj├│n',       nameKey:'libCajon',        icon: 'cx-cajon',          type: 'Caj├│n' },
  ],
  inst: [
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  PHOTO ICON MAP ΓÇö maps icon names to realistic SVG files
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
const ICON_IMAGES = {
  // ΓöÇΓöÇ Mics ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Drums ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Instruments ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Amps ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Audio / Monitors ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Utilities ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ People ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
//  Each value is innerHTML for a 24├ù24 viewBox SVG (stroke, no fill except
//  where fill="currentColor" is explicit for details like tuning pegs)
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
const CUSTOM_ICONS = {
  /* ELECTRIC BASS GUITAR ΓÇô 4-in-line headstock (right side), long slim neck,
  ...
  /* ELECTRIC GUITAR (STRATOCASTER) ΓÇô 3+3 headstock pegs (symmetric),
  ...
  /* VIOLIN / STRINGS ΓÇô small scroll, short neck, tight C-bout waist,
  ...
  /* TRUMPET / BRASS & HORNS ΓÇô horizontal: mouthpiece left, 3 valve buttons
  ...
  /* SYNTHESIZER ΓÇô outer case, 5 fader tracks (thin) + filled slider caps at
  ...
  /* ELECTRONIC DRUMS ΓÇô overhead view: octagonal main pad with trigger in
  ...
  /* PERCUSSION (CONGAS) ΓÇô two conga drums side by side.  The size difference
  ...
  /* GUITAR COMBO AMPLIFIER ΓÇô control strip with 3 knobs, single 12" speaker
  ...
  /* BASS COMBO AMPLIFIER ΓÇô same idea but taller body (bass amps are bulkier),
  ...
  /* GUITAR SPEAKER CABINET 4├ù12 ΓÇô rectangle with a 2├ù2 grid of four speakers.
     This is THE iconic look of a Marshall 4x12 ΓÇö impossible to mistake.      */
  'cx-amp-cab': `
  ...
  /* BASS SPEAKER CABINET ΓÇô wider rectangle, two large side-by-side speakers
     (4├ù10 or 2├ù15 style).  Wider speakers = lower bass suggestion.          */
  'cx-bass-cab': `
  ...
  /* FLOOR WEDGE MONITOR ΓÇô classic parallelogram/wedge profile: low front,
  ...
  /* DRUM CLIP MIC ΓÇô C-clamp/rim-mount body (left), short gooseneck curve,
  ...
  /* DUPLEX POWER OUTLET ΓÇö US-style face plate, two outlets each with
  ...
  /* WIRELESS HANDHELD MIC ΓÇö cylindrical body, antenna sticking out at side */
  'cx-wireless': `
  ...
  /* PZM / BOUNDARY MIC ΓÇö flat base plate, small capsule dome on top */
  'cx-boundary': `
    <rect x="3" y="16" width="18" height="4" rx="1.5"/>
    <path d="M9 16 Q9 11 12 11 Q15 11 15 16"/>
    <circle cx="12" cy="10" r="2" fill="currentColor"/>`,

  /* WIRELESS RACK ΓÇö rack unit with two angled antennas */
  'cx-wireless-rack': `
  ...
  /* FRONT FILL SPEAKER ΓÇö small wide low-profile cabinet angled up */
  'cx-front-fill': `
    <path d="M3 20 L21 20 L21 14 L3 17 Z"/>
    <circle cx="12" cy="17.5" r="2.5"/>
    <circle cx="12" cy="17.5" r="1" fill="currentColor"/>`,

  /* DI BOX ΓÇö compact box with XLR connectors on both sides */
  'cx-di-box': `
  ...
  /* MIC STAND ΓÇö boom stand with adjustable arm, tripod base */
  'cx-mic-stand': `
  ...
  /* PERSON / PERFORMER ΓÇö simple human silhouette */
  'cx-person': `
...
// ΓöÇΓöÇ Lucide helper: defers icon scan to after paint, optional scope ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function lcIcons(el) {
  if (!window.lucide) return;
  requestAnimationFrame(() => lucide.createIcons(el ? { el } : undefined));
}

// ΓöÇΓöÇ Custom icon renderer ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const inputSourceLabelsEs = {
  ...
  WRA:'Rack Inal├ímbrico A', WRB:'Rack Inal├ímbrico B', DNT1:'Dante Primario 01', DNT2:'Dante Primario 02', DI:'Direct Inject (DI)'
};

const GEAR_CATS_ES = {
  'Instruments':'Instrumentos','Microphones':'Micr├│fonos','Audio':'Audio',
  'Cables':'Cables','Power':'Electricidad','Outboard':'Outboard',
  'Stands':'Pedestales','Misc':'Miscel├íneos'
};

const ELEMENT_TYPES_ES = {
  'Dynamic Mic':'Micr├│fono Din├ímico','Condenser Mic':'Micr├│fono Condensador',
  'Instrument Mic':'Micr├│fono de Instrumento','Wireless Mic':'Micr├│fono Inal├ímbrico',
  'PZM Mic':'Micr├│fono PZM','Instrument Clip':'Clip de Instrumento',
  'Electric Guitar':'Guitarra El├⌐ctrica','Acoustic Guitar':'Guitarra Ac├║stica',
  'Bass Guitar':'Bajo El├⌐ctrico','Upright Bass':'Contrabajo',
  'Keyboard DI':'Teclado DI','Synthesizer':'Sintetizador',
  'Acoustic Drums':'Bater├¡a Ac├║stica','Electronic Drums':'Bater├¡a Electr├│nica',
  'Percussion':'Percusi├│n','DI Box':'Caja DI',
  'Loop Station':'Estaci├│n de Loop','Playback Device':'Dispositivo de Reproducci├│n',
  ...
  'Floor Wedge':'Monitor de Suelo (Cu├▒a)','Powered Floor PA':'PA de Suelo Amplificado',
  'Stage Sub-Woofer':'Subwoofer de Escenario','In-Ear Monitor':'Monitor In-Ear',
  'Drum Fill Monitor':'Monitor de Bater├¡a (Fill)','Drum Sub Monitor':'Subwoofer de Bater├¡a',
  ...
  'Headphone Amplifier':'Amplificador de Aud├¡fonos','Stage Mixer':'Mezclador de Escenario',
  'Power Distro':'Distribuidor de Energ├¡a','Stage Box':'Caja de Escenario',
  ...
  'Wireless Rack':'Rack Inal├ímbrico','Laptop / Computer':'Laptop / Computadora',
  'Intercom System':'Sistema Intercom'
};

// ΓöÇΓöÇ Translation helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function switchView(view) {
  ...
  if (page) {
    ...
    // context and causes them to disappear ΓÇö so Editor snaps in immediately.
...
// ΓöÇΓöÇ Mobile sidebar helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Mobile export settings sheet ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ΓöÇΓöÇ Sticky-focus killer for the left toolbar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Mobile category tray ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  (library[cat] || []).forEach(item => {
    ...
    if (item._isCreate) {
      ...
      const iconMob = item.imageData
        ...
        : `<span style="font-size:18px;line-height:1;">${item.emoji || '≡ƒÄ╡'}</span>`;
...
// ΓöÇΓöÇ Item sheet: second-level panel that replaces the bottom cat bar ΓöÇΓöÇ
function openItemSheet(cat, label) {
  ...
  (library[cat] || []).forEach(item => {
    ...
    if (item._isCreate) {
      ...
      const ico = item.imageData
        ...
        : `<span style="font-size:15px;line-height:1;">${item.emoji || '≡ƒÄ╡'}</span>`;
  ...
  // Show sheet + keep FAB in ├ù state
  setTimeout(() => {
...
// ΓöÇΓöÇ Mobile: add library item directly to center of stage ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// init Editor visible ΓÇö also show FAB and vertical toolbar for the initial view
...
function buildLibraryItem(item) {
  ...
  if (item.isCustom) {
    iconContent = item.imageData
      ...
      : `<span style="font-size:22px;line-height:1;margin-bottom:3px;">${item.emoji || '≡ƒÄ╡'}</span>`;
  } else {
  ...
  div.addEventListener('dragstart', e => {
    if (item.isCustom) {
      const payload = { name: displayName, icon: item.emoji || '≡ƒÄ╡', type: item.type || 'Custom', isCustom: true, color: item.color };
...
populateAllGrids();
// All categories start closed ΓÇö hover to open on desktop, click to toggle on mobile

// ΓöÇΓöÇ Properties panel: no hover behavior needed for top-slide panel ΓöÇΓöÇ

// ΓöÇΓöÇ Desktop flyout: hover category ΓåÆ items appear in panel beside sidebar ΓöÇΓöÇ
(function initDesktopCatBar() {
  ...
  // Trackpad / mousewheel ΓåÆ horizontal scroll (vertical deltaY converted to scrollLeft)
    ...
    list.innerHTML = DOMPurify.sanitize(roles.map(role => `
      <div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:#111;margin-bottom:3px;">
        ...
        <button onclick="removeRole('${el.id}','${role.id}')"
          style="width:18px;height:18px;display:flex;align-items:center;justify-content:center;color:#ff716c;font-size:15px;font-weight:700;background:none;border:none;cursor:pointer;flex-shrink:0;" title="Remove role">├ù</button>
      </div>
    `).join(''));
    lcIcons();
  }
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  STATUS BAR ΓÇö live plot stats
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
function updateStatusBar() {
  ...
  _sbRaf = requestAnimationFrame(() => {
    ...
    if (selEl) { const sel = state.elements.find(e => e.id === state.selectedId); selEl.textContent = sel ? sel.name : 'ΓÇö'; selEl.classList.toggle('sb-nonzero', !!sel); }
  });
}
...
function renderElements() {
  ...
  renderConnections();
  updateStatusBar();
  // Scope to the layer only ΓÇö never scan the full document (very slow)
  lcIcons(layer);
  updateDropHint();
}
...
function createElementDOM(el) {
  ...
  // Click to select / drag (pointer events ΓÇô mouse & trackpad only; touch uses touchstart below)
  wrap.addEventListener('pointerdown', e => {
    ...
    // Ctrl/Cmd+click is handled by features.js capture-phase listener ΓÇö skip here
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// ΓöÇΓöÇ Properties panel state machine ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// Drag peek ΓÇö fully hidden while moving, peek when released (unless dismissed)
...
function startDragElement(e, el) {
  ...
  // multiSel stores string IDs (from DOM attribute); state element IDs may be numbers ΓÇö compare via String()
...
//  SELECTION
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// ΓöÇΓöÇ Properties panel open / close ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function _setConnectBanner(_msg) {
  // Banner intentionally disabled ΓÇö connect mode status is shown via the
...
function handleConnectClick(id) {
  if (!state.connectSource) {
    ...
    _setConnectBanner(`FROM: ${el ? el.label : 'element'} ΓÇö click destination ┬╖ ESC to cancel`);
    renderConnections();
    return;
  }
  ...
  if (!exists) {
    ...
    showToast(`${T('connected')}: ${a ? a.label : '?'} ΓåÆ ${b ? b.label : '?'}`);
  } else {
...
let _connFP = '';
let _connHandles = [];        // { x, y, connIdx } ΓÇö rebuilt on every renderConnections()
let _cableDragState = null;   // { connIdx, prevCx, prevCy } ΓÇö set while dragging a bend handle
...
function renderConnections() {
  ...
  // Per-connection arrowhead markers ΓÇö one per unique stroke color
  ...
  // ΓöÇΓöÇ Build power-connection awareness maps ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  // Map: elementId ΓåÆ array of connection indices where this el has a power cable
  const powerConnMap = new Map(); // elId ΓåÆ count of power connections
  const powerSourceLoad = new Map(); // elId ΓåÆ count of power cables sourced from it
  ...
  // ΓöÇΓöÇ Draw visual connection lines ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  state.connections.forEach((c, idx) => {
    ...
    // Per-type visual properties ΓÇö each cable type has a distinct system color
    ...
    // Visual path ΓÇö tagged with data-vconn for hover targeting
    ...
    // Bend handle ΓÇö draggable midpoint control
    const hmt = 0.5;
    ...
    // Cable type badge ΓÇö shown only for non-XLR types
    if (cableType !== 'xlr') {
    ...
    // Destination label ΓÇö compact, placed just past the arrowhead
    const nx = dx / len, ny = dy / len;
  ...
  // ΓöÇΓöÇ Discrete power-needed indicators ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  // Only for devices that require power but have no power cable yet
  state.elements.forEach(el => {
    if (!POWER_REQUIRING_TYPES.has(el.type)) return;
    if (powerConnMap.has(el.id)) return; // already has power ΓÇö no badge
...
function _drawDiscretePowerDot(svg, x, y) {
  ...
  // Tooltip on hover ΓÇö native SVG tooltip
  const t = document.createElementNS('http://www.w3.org/2000/svg', 'title');
  t.textContent = 'Not connected to power';
  g.appendChild(t);
  // Outer glow ring ΓÇö very faint
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  CABLE CONTEXT MENU ΓÇö right-click a connection line
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓöÇΓöÇ Cable hover highlight ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Cable line bend drag + hover + right-click ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
(function _initCableInteractions() {
  document.addEventListener('DOMContentLoaded', function() {
    const canvas = document.getElementById('stage-canvas');
    if (!canvas) return;

    // Helper: client coords ΓåÆ canvas-space coords
    ...
    // ΓöÇΓöÇ Pointer-down: start bend drag when clicking a handle ΓöÇΓöÇΓöÇΓöÇ
    canvas.addEventListener('pointerdown', function(e) {
      ...
      if (handleEl && handleEl.dataset.handleConn !== undefined) {
        ...
        // Must match renderConnections() exactly ΓÇö same cap, scale, AND parity
    ...
    // ΓöÇΓöÇ Pointer-move: update bend OR hover-highlight ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    canvas.addEventListener('pointermove', function(e) {
      if (state.currentView !== 'Editor') return;
      const { cx, cy } = toCanvas(e.clientX, e.clientY);

      // (Connect mode preview disabled ΓÇö cables only render after a 2nd
      // element is tapped, so users can't see "cables to nowhere".)

      // Active bend drag ΓÇö solve for CP so the handle position EQUALS
    ...
    // ΓöÇΓöÇ Pointer-up: end drag, save ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Double-click on handle: reset bend to auto ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Right-click: cable type context menu ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Live preview line: source ΓåÆ cursor while connecting ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function renderMembersList() {
  ...
  list.innerHTML = DOMPurify.sanitize(state.members.map(m => `
    <div style="display:flex;align-items:center;gap:5px;padding:4px 6px;background:#111;border-left:2px solid ${m.color};">
      ...
      <button onclick="removeMember('${m.id}')" title="Remove" style="color:#484847;background:none;border:none;cursor:pointer;font-size:13px;padding:0 2px;line-height:1;flex-shrink:0;" onmouseover="this.style.color='#ff716c'" onmouseout="this.style.color='#484847'">├ù</button>
    </div>`).join(''));
}
...
function _repopulateMemberDropdown() {
  ...
  if (!sel) return;
  const cur = sel.value;
  sel.innerHTML = DOMPurify.sanitize('<option value="">ΓÇö None ΓÇö</option>' +
    ...
    const itemsHtml = assigned.length > 0
      ? assigned.map(el => `
          <div style="display:flex;align-items:center;gap:8px;padding:8px 14px;border-bottom:1px solid rgba(255,255,255,0.04);">
            ...
            <span style="font-size:11px;color:#d4d4d4;font-weight:500;font-family:'Inter',sans-serif;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${el.label || el.name || 'ΓÇö'}</span>
...
function refreshRider() {
  ...
  if (elems.length === 0) {
    ...
    emptyTd.innerHTML = '<div style="font-family:Manrope,sans-serif;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.18em;color:#484847;">' + (T('noElemsRider') || 'No stage elements yet') + '</div><div style="font-size:10px;color:#333;margin-top:5px;font-family:Inter,sans-serif;">Add elements to the stage ΓÇö they appear here automatically</div>';
    ...
    sorted.forEach(function(el, i) {
      ...
      td2.textContent = el.label || el.name || 'ΓÇö';
      td2.setAttribute('data-label', 'Source Name');
      var td3 = document.createElement('td');
      td3.innerHTML = '<span class="rd-type-badge">' + (el.type || el.name || 'ΓÇö') + '</span>';
    ...
    <div style="border:1px solid var(--rn-border);background:var(--rn-card-bg);color:var(--rn-text);border-radius:8px;margin-bottom:8px;overflow:hidden;transition:border-color 0.2s;">
      <div style="display:flex;align-items:center;gap:8px;padding:10px 12px 6px;">
        ...
        <button onclick="removeRiderNeed('${need.id}')"
          ...
          onmouseout="this.style.color='var(--rn-muted)';this.style.borderColor='var(--rn-border)';this.style.background='none'">├ù</button>
      </div>
...
function renderSetlist() {
  ...
  if (!state.setlist.length) {
    tbody.innerHTML = DOMPurify.sanitize(`<div style="padding:56px 0;text-align:center;">
      <div style="font-family:'Manrope',sans-serif;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.2em;color:#484847;">No songs yet ΓÇö tap ADD NEW TRACK to start your setlist.</div>
    </div>`);
  ...
  state.setlist.forEach((item, i) => {
    if (item.type === 'section') {
      ...
      const chipDur = (item.duration && item.duration !== 'ΓÇö') ? `<span class="sl-chip-dur">${item.duration}</span>` : '';
      const safeNotes = (item.notes || '').replace(/</g,'&lt;').replace(/>/g,'&gt;');
      const notesHtml = item.notes
        ? `<p class="sl-notes-txt">${safeNotes}</p>`
        : `<p class="sl-notes-txt sl-notes-empty">No notes ΓÇö edit song to add cues</p>`;
      html += `
...
// ΓöÇΓöÇ Sections (inline setlist dividers) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const _SL_SECTION_TYPES = {
  ...
  main:    { name: 'Main Set',        description: 'The core journey ΓÇö 3ΓÇô4 song clusters alternating high & low energy.', color: '#7aafff' },
...
// ΓöÇΓöÇ Drag-to-reorder (HTML5 desktop) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Drag-to-reorder (touch / mobile) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function _renderSegmentsBar() {
  ...
  bar.innerHTML = DOMPurify.sanitize(segs.map(seg => {
    ...
    return `<div style="display:flex;align-items:center;gap:6px;padding:5px 10px;background:${seg.color}18;border:1px solid ${seg.color}44;">
      ...
      <button onclick="removeSegment(${seg.id})" style="background:none;border:none;color:#484847;cursor:pointer;padding:0;margin-left:2px;line-height:1;font-size:13px;" onmouseover="this.style.color='#ff716c'" onmouseout="this.style.color='#484847'" title="Remove segment">├ù</button>
    </div>`;
  }).join(''));
}
...
function renderSetlistInsights() {
  ...
  if (!songs.length) {
    tEl.textContent = 'ΓÇö'; tEl.style.color = '';
    kEl.textContent = 'ΓÇö'; kEl.style.color = '';
    fEl.textContent = 'ΓÇö'; fEl.style.color = '';
    return;
  }

  // ΓöÇΓöÇ Tempo Stability ΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Transition Fluidity ΓöÇΓöÇ
  if (songs.length < 2) {
...
function saveSong() {
  ...
  const newSong = {
    ...
    duration:  document.getElementById('sng-duration').value.trim() || 'ΓÇö',
...
// ΓöÇΓöÇΓöÇ BATCH IMPORT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function _parseSongLines(text) {
  return text.split('\n')
    ...
    .filter(l => l.length > 0)
    .map(l => {
      // Strip leading numbers/bullets: "1.", "1)", "-", "ΓÇó", "Γû╕", "*", "ΓÇô"
      l = l.replace(/^\d+[.)]\s*/, '').replace(/^[-ΓÇó*ΓÇôΓû╕Γû║Γû╢┬╖]\s*/, '').trim();
      ...
      // Extract musical key: "Am", "Bb", "F#m", "Ebm" ΓÇö but only strip it if
      // it appears after a known separator, so we don't eat part of the title
      const keyM = l.match(/(?:[-ΓÇôΓÇö|,┬╖()\s])\s*([A-G][#b]?m?)\s*(?:[-ΓÇôΓÇö|,┬╖()\s]|$)/);
      if (keyM) { key = keyM[1]; l = l.replace(keyM[0], ' '); }

      // Clean up leftover separators and trailing/leading whitespace
      const title = l.replace(/\s*[-ΓÇôΓÇö|,┬╖()]\s*$/g, '').replace(/^\s*[-ΓÇôΓÇö|,┬╖()]\s*/g, '').replace(/\s+/g, ' ').trim();
...
function updateBatchPreview() {
  ...
  if (songs.length > 0) {
    ...
    preview.textContent = text.trim() ? 'No songs detected ΓÇö check format' : '';
...
async function handleBatchFile(input) {
  ...
  if (file.name.endsWith('.pdf') || file.type === 'application/pdf') {
    const preview = document.getElementById('batch-preview-count');
    preview.textContent = 'Extracting text from PDFΓÇª';
    ...
    try {
      ...
      preview.textContent = 'Could not read PDF ΓÇö try copy-pasting the text instead';
      preview.style.color = '#ff6b6b';
    }
    return;
  }

  document.getElementById('batch-preview-count').textContent = 'Unsupported file type ΓÇö use .txt or .pdf';
...
// ΓöÇΓöÇ SEGMENT MODAL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ SMART SORT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
let _smartSortProposal = null; // { setlist, segments }

// ΓöÇΓöÇ BPM ΓåÆ energy (0-100). Primary energy source for Smart Sort ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Classic live-show energy arc (position 0ΓåÆ1) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Greedy sort: fill each slot with the best-scoring remaining song ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Build proposed segments without touching state ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function openSmartSortModal() {
  ...
  // Always sort the FULL list ΓÇö auto-segments override existing ones
  ...
  const desc = `Songs sorted by BPM-driven energy arc ΓÇö strong opener, building tension, peak zone, breather, then big finale. Auto-split into ${segCount} segments (${use4 ? 'Opener / Build / Main Set / Encore' : 'Opener / Main Set / Encore'}) based on position. Existing segments will be replaced. PDF will be exported automatically.`;
  ...
  sorted.forEach((song, i) => {
    ...
    const seg = proposedSegs.find(s => s.id === curSegId);
    const derived = _derivedEnergy(song);
    const bpmLabel = song.bpm ? `${song.bpm} BPM` : 'ΓÇö BPM';
...
async function applySmartSort() {
  ...
  renderSetlist();
  pushHistory();
  showToast('Smart Sort applied ΓÇö generating PDFΓÇª');
...
function loadPreset(id) {
  ...
  showConfirm(Tformat('loadPresetConfirm', { name: p.name }), () => {
    ...
    updateDropHint();
    // Reset history ΓÇö undo should not cross a preset-load boundary
...
// ΓöÇΓöÇ Quick Presets ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const QUICK_PRESETS = {
  rock: {
    name: 'Rock Band',
    elements: [
  ...
  if (!filtered.length) {
    ...
    container.innerHTML = DOMPurify.sanitize(filtered.map(g => {
      ...
      return catHdr + `<div class="${rowCls}" data-gear-id="${g.id}">
        ...
        <button class="gear-item-del" onclick="deleteGearItem(${g.id})">├ù</button>
      </div>`;
    }).join(''));
  }
...
function refreshExportGear() {
  ...
  tbody.innerHTML = DOMPurify.sanitize(sorted.map((g, i) => {
    ...
    return catRow + `<tr style="background:${bg};">
      ...
      <td style="padding:7px 12px;font-size:10px;color:#767575;">${g.notes ? escapeHtml(g.notes) : 'ΓÇö'}</td>
      <td style="padding:7px 12px;text-align:center;">
        <div style="width:14px;height:14px;border:1.5px solid ${g.packed ? '#c5ffc9' : '#484847'};background:${g.packed ? '#c5ffc9' : 'transparent'};margin:0 auto;display:flex;align-items:center;justify-content:center;">
          ${g.packed ? '<span style="font-size:9px;color:#002e00;line-height:1;">Γ£ô</span>' : ''}
        </div>
      </td>
    </tr>`;
  }).join(''));
...
//  AUTOSAVE
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
let _sessionDirty = false;   // true after any user change ΓÇö triggers reload warning
...
function renderMobileScenesList() {
  ...
  if (addBtnLabel) {
    addBtnLabel.textContent = state.lang === 'es' ? 'A├▒adir escena' : 'Add Scene';
  }
...
function duplicateScene(idx) {
  _ensureScenes();
  if (state.scenes.length >= SCENES_MAX) {
    showToast(state.lang === 'es' ? 'M├íximo 3 escenas' : 'Max 3 scenes');
    return;
  }
  ...
  const tabsHtml = state.scenes.map((s, i) => {
    ...
    return `
      <button onclick="switchScene(${i})" title="${s.name}"
        ...
        <span>${s.name}</span>
        ${state.scenes.length > 1 ? `<span onclick="event.stopPropagation();removeScene(${i})" class="sc-scene-close">├ù</span>` : ''}
      </button>`;
  }).join('');
  
  const addHtml = state.scenes.length < SCENES_MAX
    ? `<button onclick="addScene()" title="${state.lang === 'es' ? 'A├▒adir escena' : 'Add scene'}" class="sc-scene-add-btn">
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SCENES (v3.0.63+) ΓÇö up to 3 stage-plot scenes per project
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
const SCENES_MAX = 3;
...
function switchScene(idx) {
  ...
  // layout ΓÇö history is scoped to the active scene's lifetime.
  state.history = [];
...
function removeScene(idx) {
  ...
  const body = state.lang === 'es' 
    ? 'Esto eliminar├í esta escena de tu plano de escenario. Esta acci├│n no se puede deshacer.'
...
// ΓöÇΓöÇ Silent full-session persistence ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function loadSaved() {
  try {
    ...
    // ΓöÇΓöÇ Scenes (schemaVersion 9+) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    // Older saves had no scenes array ΓÇö wrap the loaded
...
// ΓöÇΓöÇ Scene info bridge for the React PDF sheet ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// (wrap, layer) pair. Pure function of `scene` ΓÇö does not touch
...
async function exportPDF() {
  ...
  // the "GENERATINGΓÇª" label if export runs longer than 400ms ΓÇö most
  ...
  const spinnerTimer = genBtn ? setTimeout(() => {
    genBtn.innerHTML = DOMPurify.sanitize('<span style="font-size:11px;letter-spacing:0.1em;">GENERATINGΓÇª</span>');
  }, 400) : null;
  ...
  try {
    ...
    // clone in onclone ΓÇö those are extremely expensive to rasterize and
    ...
    // ΓöÇΓöÇ 1. Clone the WHOLE export document once ΓöÇΓöÇ
    const wrap = document.createElement('div');
    wrap.style.cssText =
    ...
    // ΓöÇΓöÇ Scene-aware stage section(s) for PDF export ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    // The live #exp-stage-section reflects the active scene. If the
    // user picked a different single scene or "all scenes", swap the
    // clone's stage section(s) accordingly. We mutate ONLY docClone ΓÇö
    ...
    // ΓöÇΓöÇ 2. One animation frame so layout settles ΓöÇΓöÇ
    await new Promise(r => requestAnimationFrame(r));

    // ΓöÇΓöÇ 3. Record section break points + total height BEFORE capture ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ 4. Try the fast single-capture path. Cleanup is guaranteed in
    ...
    // Most browsers cap canvas height around 16kΓÇô32k px. Stay well under.
    ...
    // ΓöÇΓöÇ PDF document ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    if (bigCvs) {
      // ΓöÇΓöÇ 5a. FAST PATH: slice the single big canvas into A4 pages,
      ...
      // ΓöÇΓöÇ 5b. FALLBACK PATH: capture each section individually.
    ...
    if (opts.action === 'share' && typeof navigator !== 'undefined' && typeof navigator.share === 'function') {
      ...
      try {
        ...
        if (shareErr && shareErr.name === 'AbortError') {
          // User dismissed the native share sheet ΓÇö leave the file unsaved.
          shared = true;
        }
      }
      if (!shared) {
        ...
        } catch (_) { /* ignored ΓÇö saving below */ }
        pdf.save(fileName);
      }
...
// ΓöÇΓöÇ Bottom-bar chip toggle ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Export bar scroll-hide wiring ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function shareLink() {
...
function refreshExport() {
  ...
  if (elStatus) {
    ...
    elStatus.textContent = n + ' ' + (n === 1 ? T('elementSingular') : T('elements')) + ' ΓÇö ' + T('elementsOnStage');
  }
  ...
  // ΓöÇΓöÇ Last Updated / Version ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  if (showLU && state.lastModified) {
    ...
    const luStr = lm.toLocaleDateString(locale, { month: 'short', day: '2-digit', year: 'numeric' })
                + ' ΓÇô ' + lm.toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit', hour12: false });
...
function refreshExportCanvas() {
  ...
  // Draw connection lines over the mini stage ΓÇö clean curved style matching the editor
  if (state.connections.length > 0) {
    ...
    state.connections.forEach((c, idx) => {
      ...
      // Convert absolute px coords ΓåÆ percentage coords inside the mini canvas
      const x1 = a.x / refW * 100, y1 = a.y / refH * 100;
      const x2 = b.x / refW * 100, y2 = b.y / refH * 100;
      const dx = x2 - x1, dy = y2 - y1;
      const len = Math.sqrt(dx*dx + dy*dy) || 1;
...
function refreshExportInputList() {
  ...
  const rowsHtml = sorted.map((el, i) => {
    const micDI = Ttype(el.type || el.name) || 'ΓÇö';
    const source = TSource(el.source) || el.source || 'ΓÇö';
    ...
    const performerCell = member
      ...
      : '<span style="color:#484847;font-size:12px;">ΓÇö</span>';
    let rows = `
    <tr style="background:${i % 2 === 0 ? '#131313' : '#0e0e0e'};">
      <td style="padding:10px 14px;font-family:'Manrope',sans-serif;font-size:15px;font-weight:700;color:#7aafff;">${el.channelId || 'ΓÇö'}</td>
      ...
      <td style="padding:10px 14px;color:#767575;font-size:12px;font-style:italic;">${el.notes || 'ΓÇö'}</td>
    </tr>`;
    (el.roles || []).forEach(role => {
      const roleSrc = TSource(role.source) || role.source || 'ΓÇö';
      rows += `
      <tr style="background:${i % 2 === 0 ? '#0f0f0f' : '#0a0a0a'};">
        <td style="padding:6px 14px 6px 26px;font-family:'Manrope',sans-serif;font-size:12px;color:#7aafff;">
          <span style="color:#333;margin-right:4px;">Γå│</span>${role.channelId || 'ΓÇö'}
        </td>
        <td style="padding:6px 14px 6px 26px;font-weight:500;color:#adaaaa;font-size:11px;">${role.name}</td>
        <td style="padding:6px 14px;color:#484847;font-size:11px;">ΓÇö</td>
        ...
        <td style="padding:6px 14px;color:#484847;font-size:11px;font-style:italic;">${role.notes || 'ΓÇö'}</td>
      </tr>`;
    });
    return rows;
  }).join('');
    ...
    const patchRows = connCount > 0
      ? state.connections.map((c, i) => {
          ...
          return `<div style="display:flex;align-items:center;gap:12px;padding:7px 12px;background:${i % 2 === 0 ? '#111' : '#131313'};border-left:2px solid rgba(122,175,255,0.25);">
            ...
            <span style="font-size:14px;color:#7aafff;flex-shrink:0;font-weight:900;">Γƒ╢</span>
  ...
  if (state.riderNeeds.length > 0) {
    const cards = state.riderNeeds.map(need => {
      ...
      return `<div style="padding:12px 14px;background:#131313;border-left:2px solid ${nt.color};">
        ...
        <p style="font-size:12px;color:#adaaaa;margin:0;line-height:1.7;word-break:break-word;overflow-wrap:break-word;">${need.value || 'ΓÇö'}</p>
      </div>`;
    }).join('');
    reqsHtml = `
      <div>
...
// causes forced layout at 500+ fps ΓÇö throttle to one read per animation frame
let _mmRaf = null;
...
// (beforeunload warning removed ΓÇö reload without prompting)
...
function loadSettings() {
  try {
    ...
    /* showStatusBar intentionally not restored ΓÇö always hidden in embedded view */
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö apply to DOM
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// ΓöÇΓöÇ Export navigation helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function _initExportScrollHide() {
  ...
  const update = () => {
    ...
    if (dy > 0) {
      // Scrolling down ΓÇö hide immediately and reset the upward counter.
      upAccum = 0;
      post(true);
    } else if (dy < 0) {
      // Scrolling up ΓÇö only show after enough sustained upward motion.
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö nav drag-and-drop list
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö canvas background
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö status bar toggle
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö snap toggle (from settings)
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö connections toggle (from settings)
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö labels visibility toggle
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö connection line style
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö animations toggle
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
function backupJSON() {
  ...
  showToast('Backup downloaded Γ£ô');
}
    ...
    reader.onload = ev => {
      try {
        ...
        showConfirm('Restore from this backup? Current project will be replaced.', () => {
          ...
          showToast('Backup restored Γ£ô');
        });
      } catch {
        showToast('Invalid file ΓÇö could not parse JSON');
      }
    };
    reader.readAsText(file);
  };
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SETTINGS ΓÇö sync UI to current state
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
function _applyCloudData(d) {
  ...
  // ΓöÇΓöÇ Scenes (schemaVersion 9+) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
const SMART_RULES = {
  'Acoustic Drums':     { text: 'Full kit typically needs 6ΓÇô8 inputs: Kick, Snare, Hi-Hat, Tom ├ù2, Overhead L/R ΓÇö and optionally a Room mic.', apply: 'drums' },
  'Electronic Drums':   { text: 'E-drum brain outputs direct to DI ΓÇö usually stereo (2 ch, L+R). Trigger pads may add extra outputs. Confirm with drummer.' },
  ...
  'Acoustic Guitar':    { text: 'Acoustic pickup ΓåÆ DI box. Some systems need phantom power (+48V). Blending a condenser mic gives a fuller, more natural sound.' },
  ...
  'Keyboard DI':        { text: 'Stereo DI recommended ΓÇö confirms 2 channels (L + R). Verify with the player whether they output mono or stereo.', apply: 'keys' },
  'Synthesizer':        { text: 'Many synths have multiple outputs. Confirm mono or stereo ΓÇö stereo needs 2 channels. Some setups have separate sub-mixes.' },
  ...
  'String Instrument':  { text: 'Condenser mic or pickup DI. Confirm active or passive pickup ΓÇö active may need phantom power (+48V).' },
  ...
  'Caj├│n':              { text: 'A single close mic or kick-style dynamic works great. Some players use a DI pickup mounted inside the caj├│n body.' },
...
function acceptSuggestion() {
  ...
  if (rule.apply === 'drums') {
    ['Kick mic', 'Snare mic', 'Hi-Hat mic', 'Tom L mic', 'Tom R mic', 'Overhead L', 'Overhead R'].forEach(n => {
      state.riderNeeds.push({ id: 'rn' + state._rnNextId++, type: 'custom', value: el.label + ' ΓÇö ' + n });
    });
    renderRiderNeeds();
    showToast('Drum mic layout added to Technical Requirements');
  } else if (rule.apply === 'elec-guitar') {
    state.riderNeeds.push({ id: 'rn' + state._rnNextId++, type: 'custom', value: el.label + ' ΓÇö Amp mic or DI box' });
    renderRiderNeeds();
    showToast('Guitar setup note added to Technical Requirements');
  } else if (rule.apply === 'bass') {
    state.riderNeeds.push({ id: 'rn' + state._rnNextId++, type: 'custom', value: el.label + ' ΓÇö Bass DI (direct)' });
    renderRiderNeeds();
    showToast('Bass DI note added to Technical Requirements');
  } else if (rule.apply === 'keys') {
    state.riderNeeds.push({ id: 'rn' + state._rnNextId++, type: 'custom', value: el.label + ' ΓÇö Stereo DI (L + R, 2 channels)' });
    renderRiderNeeds();
...
function toggleGigMode() {
  ...
  if (!state.gigMode) {
    // ΓöÇΓöÇ Entering focus mode ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Exiting focus mode ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // 2. Next paint: remove gig-fade-out ΓåÆ panels fade back in
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        body.classList.remove('gig-fade-out');
        state.gigMode = false;
...
function openCustomElementModal() {
  ...
  document.getElementById('cust-el-emoji').value = '≡ƒÄ╡';
...
function saveCustomElement() {
  ...
  const emoji    = (document.getElementById('cust-el-emoji').value || '').trim() || '≡ƒÄ╡';
...
function _renderCustomElementsLibraryImpl() {
  ...
  items.filter(item => !item.category || item.category === 'custom').forEach(item => {
    ...
    div.innerHTML = DOMPurify.sanitize(`
      ...
      <button onclick="event.stopPropagation();deleteCustomElement('${item.id}')" title="Remove" style="position:absolute;top:1px;right:2px;background:none;border:none;cursor:pointer;color:#3a3a3a;font-size:12px;padding:0;line-height:1;" onmouseover="this.style.color='#ff716c'" onmouseout="this.style.color='#3a3a3a'">├ù</button>`);
    div.addEventListener('dragstart', e => {
...
function _buildInputListItems() {
  ...
  state.elements.forEach(el => {
    ...
    if (!count) return;
    if (count === 1) {
      newNeeds.push({ id: 'rn' + state._rnNextId++, type: 'custom', value: (el.label || el.name) + ' ΓÇö ' + el.type + ' (' + el.channelId + ')' });
    } else {
      for (let i = 1; i <= count; i++) {
        ...
        if (el.type === 'Keyboard DI' || el.type === 'Synthesizer' || el.type === 'Electronic Drums') suffix = i === 1 ? ' L' : ' R';
          ...
          suffix = ' ΓÇö ' + (drumSuffix[i-1] || 'Ch ' + i);
        }
...
// ΓöÇΓöÇ Auto-sync balance after any drag-move event on stage ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
(function() {
  let _balRaf = null;
...
// Zone definitions: row (0=upstage ΓÇª 4=downstage), side bias
const _ARRANGE_ZONES = {
  // Upstage row ΓÇô rear tech / PA
  ...
  // Mid-back row ΓÇô amps & backline
...
// inner third ΓÇö which made everything look bunched toward the middle.
function _smartPlaceElements(elements, W, H) {
  ...
  // Row Y fractions ΓÇö well-spread for a real stage
  ...
  rows.forEach((sides, rowIdx) => {
    ...
    // ΓöÇΓöÇ Edges: alternate left / right at the outermost positions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    sides.edges.forEach((el, i) => {
      el.x = i % 2 === 0 ? PAD_X : W - PAD_X;
      el.y = baseY;
    });

    // ΓöÇΓöÇ Spread: items use the full stage width ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    placeSpread(sides.spread, baseY);

    // ΓöÇΓöÇ L / C / R via anchor placement. If a row also has spread items,
    ...
    // ΓöÇΓöÇ Collision nudge: push any same-row pair closer than MIN_SEP ΓöÇΓöÇ
    const allInRow = [
...
function _doAutoArrange() {
  ...
  showToast('Stage arranged Γ£ô ΓÇö Undo to revert.');
}
...
function renderTimeline() {
  ...
  bars.innerHTML = DOMPurify.sanitize(items.map(it => {
    ...
    return `<div style="height:100%;background:${col};opacity:0.45;flex:${secs || 1};min-width:4px;" title="${it.name} ┬╖ ${it.duration || 'ΓÇö'}"></div>`;
  }).join(''));
  ...
  list.innerHTML = DOMPurify.sanitize(items.map((it, i) => {
    ...
    return `<div class="tl-item" draggable="true"
      ...
      <div class="tl-item-actions">
        <button class="tl-act-btn" onclick="editTlItem(${i})" title="Edit">Γ£Ä</button>
        <button class="tl-act-btn del" onclick="deleteTlItem(${i})" title="Delete">Γ£ò</button>
      </div>
    </div>`;
  }).join(''));
}
...
function syncTimelineFromSetlist() {
  if (!state.setlist.length) { showToast('Setlist is empty ΓÇö add songs there first.'); return; }
  ...
  state.setlist.forEach(song => {
    ...
    if (!existing.has(name)) {
      state.timeline.push({
        ...
        type: 'song',
        name,
        duration: song.duration && song.duration !== 'ΓÇö' ? song.duration : '',
...
function generateShareLink() {
  ...
  try {
    ...
    showToast('Failed to encode project ΓÇö project may be too large.'); return;
  }
  ...
  showToast('Share link generated ΓÇö copy it!');
}
...
// ΓöÇΓöÇ QR Code ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

let _qrInstance = null;

function generateQRCode() {
  if (!_shareUrl) { showToast('Generate a share link first.'); return; }
  if (typeof QRCode === 'undefined') { showToast('QR library not loaded yet ΓÇö try again.'); return; }
...
// ΓöÇΓöÇ On-load: detect share link in URL hash ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
(function _detectShareMode() {
  ...
  try {
    ...
    // ΓöÇΓöÇ Block ALL state-modifying interaction ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Cloud sync bridge ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// Receives postMessage from the parent (Chordex shell) to snapshot or restore
// StageX localStorage. Audio blobs/IndexedDB are NOT included ΓÇö only the JSON
// state listed below.
(function () {
  var SYNC_KEYS = [
    'stagecoreProject',
