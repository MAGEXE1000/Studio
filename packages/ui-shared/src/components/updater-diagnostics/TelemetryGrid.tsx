...
import { APP_VERSION, globalUpdateState, updateDebugLogs, updateDiagnostics, isNative, PerformanceProfiler } from '@workspace/studio-core';
...
export default function TelemetryGrid({ nativeDeviceInfo, nativeInstallerDetails }: TelemetryGridProps) {
  ...
  const installedVersionCode = getDisplayValue(updateDebugLogs.installedVersionCode, 'Not initialized');
  ...
  const storageAvailable = getDisplayValue(nativeDeviceInfo?.storageAvailable || updateDiagnostics?.storageAvailable, 'Not initialized');
  const networkState = getDisplayValue(nativeDeviceInfo?.networkState || updateDiagnostics?.networkState, 'Not initialized');
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-black">
      ...
      <div className="bg-black p-4 rounded-xl flex flex-col gap-1 border border-outline-variant/10 col-span-2 md:col-span-4">
        ...
        <span className="text-sm font-bold text-on-surface mt-1">
          Render Count: <span className="text-tertiary">{updateDebugLogs.renderCount || 0}</span> &bull; Paint Count: <span className="text-tertiary">{updateDebugLogs.paintCount || 0}</span> &bull; Layout Count: <span className="text-tertiary">{updateDebugLogs.layoutCount || 0}</span>
        </span>
      </div>
