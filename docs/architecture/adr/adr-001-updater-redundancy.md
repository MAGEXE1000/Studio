...
- **Forced web download immediately**: Prompting users to open a web browser as soon as the first download fails.
  - _Why Rejected_: Poor user experience. The app should always attempt a seamless, in-app update before requesting manual web browsing actions.
- **Automatic cache clearance on every retry**:
  - _Why Rejected_: Discards partial downloads, defeating the purpose of Range-based smart resume.
