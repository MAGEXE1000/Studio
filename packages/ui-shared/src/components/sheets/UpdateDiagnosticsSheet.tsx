...
export function getStructuredDiagnostics(developerMode: boolean): DiagnosticEntry[] {
  ...
  // 1. PERFORMANCE
  const boot = (typeof window !== 'undefined' ? (window as any).__bootTimings : null) || {
    introStart: 0,
    hubVisible: 0,
  };
  const natBoot = (typeof window !== 'undefined' ? (window as any).__nativeBootTimings : null) || {
    processStart: 0,
    onCreate: 0,
    webViewInit: 0,
  };
  ...
  const jsInitMs =
    natBoot.webViewInit && natBoot.onCreate ? natBoot.webViewInit - natBoot.onCreate : 0;
  ...
  list.push({
    ...
    humanExplanation:
      'The time taken by the Android operating system to initialize the app process.',
  ...
  list.push({
    ...
    humanExplanation:
      'How quickly the application renders its first visual pixel (the planets intro).',
...
export default function UpdateDiagnosticsSheet({ open, onClose }: Props) {
  ...
  const diagnosticsList = useMemo(
    () => getStructuredDiagnostics(settings.developerMode ?? false),
  ...
  const timelineStates = [
    ...
    { id: 'VALIDATE_METADATA', label: 'Validate Manifest', desc: 'Parsing remote JSON metadata' },
    {
      ...
      desc: 'Evaluating installed vs target version',
    },
    ...
    {
      id: 'PREPARING_INSTALL',
      label: 'Preparing Install',
      desc: 'Verifying certificate signing match',
    },
    {
      id: 'WAITING_USER_CONFIRMATION',
      label: 'Awaiting User',
      desc: 'Awaiting install user action prompt',
    },
    {
      id: 'PACKAGEINSTALLER_VISIBLE',
      label: 'Installer Active',
      desc: 'PackageInstaller activity in foreground',
    },
  ...
  const handleVerifyCurrentApk = async () => {
    ...
    const { runEligibilityCheck } =
      await import('@workspace/studio-core/src/lib/updater/eligibilityVerification');
    const eligible = await runEligibilityCheck(path);
    alert(
      eligible
        ? 'APK integrity and verification success!'
        : `APK verification failed: ${updateDebugLogs.eligibilityReason}`
    );
  };
  ...
  const handleValidateDownloadUrl = async () => {
    ...
    try {
      ...
      alert(
        res.ok
          ? `URL reachable! Returned status: ${res.status}`
          : `URL verification failed: HTTP ${res.status}`
      );
    } catch (e) {
        ...
        >
          ...
          <header className="w-full shrink-0 flex items-center justify-between px-6 py-4 border-b border-[#484848]/20 bg-[#0e0e0e]/85 backdrop-blur-md sticky top-0 z-[10000]">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-[17px] font-extrabold text-[#e7e5e4] tracking-tight m-0 leading-tight">
                  Updater Diagnostics
                </h1>
                <p className="text-[10px] text-[#acabaa] font-bold uppercase tracking-widest m-0 mt-0.5">
          ...
          <main className="flex-1 overflow-y-auto px-6 py-6 max-w-4xl w-full mx-auto space-y-6 box-border pb-32">
            ...
            >
              <motion.div
                ...
                className="bg-[#191a1a] p-4 rounded-xl flex flex-col gap-1 border border-[#484848]/10"
              >
                <span className="text-[#acabaa] text-[9px] uppercase tracking-widest font-black">
                  App Version
                </span>
              ...
              >
                ...
                <span className="text-[16px] font-extrabold text-[#e7e5e4]">
                  {ota.remoteVersion || 'N/A'}
                </span>
              </motion.div>
              <motion.div
                variants={{ hidden: { opacity: 0, y: 15 }, show: { opacity: 1, y: 0 } }}
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2.5 h-2.5 rounded-full ${ota.loading ? 'bg-[#679cff] animate-pulse' : 'bg-green-500'}`}
                  />
                  <span className="text-[15px] font-extrabold text-[#e7e5e4]">
                    {ota.loading ? 'Updating' : 'Idle'}
                  </span>
                </div>
              </motion.div>
              <motion.div
              ...
              >
                ...
                <span
                  className="text-[14px] font-extrabold text-[#e7e5e4] truncate"
                  title={ota.updateState}
                >
                  {ota.updateState}
                </span>
              </motion.div>
              ...
              <motion.div
                variants={{ hidden: { opacity: 0, y: 15 }, show: { opacity: 1, y: 0 } }}
                className="col-span-2 md:col-span-4 bg-[#191a1a] p-5 rounded-xl border border-[#484848]/10"
              >
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#191a1a]/40 rounded-2xl border border-[#484848]/10 overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-[#acabaa] text-lg">
                      settings_suggest
                    </span>
                    <span className="text-[11px] font-black text-[#acabaa] uppercase tracking-widest">
                      Production Actions
                    </span>
                  </div>
                  <span
                    className={`material-symbols-outlined text-[#acabaa] transition-transform duration-250 ${!collapsed.actions ? 'rotate-180' : ''}`}
                  >
                    expand_more
                  </span>
                </button>
                  ...
                  <div className="p-4 pt-0 border-t border-[#484848]/5 bg-[#0e0e0e]/25">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3">
                      ...
                      >
                        ...
                        <div>
                          <span className="block font-bold text-xs text-[#e7e5e4]">
                            Check for Updates
                          </span>
                          <span className="text-[10px] text-[#acabaa]">
                            Poll registries manually
                          </span>
                        </div>
                      </button>
              ...
              <div className="bg-[#191a1a]/40 rounded-2xl border border-[#484848]/10 overflow-hidden">
                <button
                  onClick={() =>
                    setCollapsed((prev) => ({ ...prev, simulation: !prev.simulation }))
                  }
                  className="w-full flex items-center justify-between p-4 bg-transparent border-none text-left cursor-pointer hover:bg-[#1f2020] transition-colors focus:outline-none"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-black text-[#acabaa] uppercase tracking-widest">
                      Interactive Engineering Actions
                    </span>
                  </div>
                  <span
                    className={`material-symbols-outlined text-[#acabaa] transition-transform duration-250 ${!collapsed.simulation ? 'rotate-180' : ''}`}
                  >
                    expand_more
                  </span>
                </button>
                ...
                {!collapsed.simulation && (
                  <div className="p-4 pt-0 border-t border-[#484848]/5 bg-[#0e0e0e]/25">
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-3">
                      <button
                        onClick={handleCheckUpdates}
                        className="bg-[#191a1a] hover:bg-[#252626] border border-[#484848]/10 text-left p-3 rounded-lg text-xs font-bold text-[#e7e5e4] focus:outline-none cursor-pointer transition-colors"
                      >
                        Refresh Metadata
                      </button>
                      <button
                        onClick={handleVerifyCurrentApk}
                        className="bg-[#191a1a] hover:bg-[#252626] border border-[#484848]/10 text-left p-3 rounded-lg text-xs font-bold text-[#e7e5e4] focus:outline-none cursor-pointer transition-colors"
                      >
                        Verify Current APK
                      </button>
                      ...
                      <button
                        onClick={handleValidateDownloadUrl}
                        className="bg-[#191a1a] hover:bg-[#252626] border border-[#484848]/10 text-left p-3 rounded-lg text-xs font-bold text-[#e7e5e4] focus:outline-none cursor-pointer transition-colors"
                      >
                    ...
                    <div className="mt-4 pt-4 border-t border-[#484848]/10">
                      <span className="block text-[9px] font-black uppercase text-[#acabaa] tracking-wider mb-2">
                        Simulation Lab Toggles
                      </span>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div className="flex items-center justify-between bg-[#191a1a] p-3 rounded-lg border border-[#484848]/10">
                          ...
                          >
                            <div
                              className={`w-3 h-3 bg-white rounded-full transition-transform ${simulationList.forceUpdateAvailable ? 'translate-x-4' : 'translate-x-0'}`}
                            />
                          </button>
                        </div>
                        <div className="flex items-center justify-between bg-[#191a1a] p-3 rounded-lg border border-[#484848]/10">
                          <span className="text-xs font-bold text-[#ee7d77]">
                          ...
                          >
                            <div
                              className={`w-3 h-3 bg-white rounded-full transition-transform ${simulationList.forceDownloadFailure ? 'translate-x-4' : 'translate-x-0'}`}
                            />
                          </button>
                        </div>
                        <div className="flex items-center justify-between bg-[#191a1a] p-3 rounded-lg border border-[#484848]/10">
                          ...
                          >
                            <div
                              className={`w-3 h-3 bg-white rounded-full transition-transform ${simulationList.forceSignatureMismatch ? 'translate-x-4' : 'translate-x-0'}`}
                            />
                          </button>
                        </div>
                        <div className="flex items-center justify-between bg-[#191a1a] p-3 rounded-lg border border-[#484848]/10">
                          ...
                          >
                            <div
                              className={`w-3 h-3 bg-white rounded-full transition-transform ${simulationList.simulateDownloadThrottling ? 'translate-x-4' : 'translate-x-0'}`}
                            />
                          </button>
                        </div>
                      </div>
              ...
              <div className="bg-[#191a1a]/40 rounded-2xl border border-[#484848]/10 overflow-hidden">
                ...
                >
                  ...
                  <span
                    className={`material-symbols-outlined text-[#acabaa] transition-transform duration-250 ${!collapsed.logs ? 'rotate-180' : ''}`}
                  >
                    expand_more
                  </span>
                </button>
                    ...
                    <div className="bg-[#000000] border border-[#484848]/10 rounded-xl overflow-hidden flex flex-col h-80 shadow-inner mt-3">
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-2 bg-[#1f2020]/60 border-b border-[#484848]/10 shrink-0">
                        <div className="flex-1 flex items-center gap-2 bg-[#131313] px-3 py-1.5 rounded-lg border border-[#484848]/10">
                          <span className="material-symbols-outlined text-sm text-[#acabaa]">
                            search
                          </span>
                          <input
                            type="text"
                      ...
                      <div className="flex-1 p-4 font-mono text-[10px] leading-relaxed overflow-y-auto space-y-1.5 selection:bg-[#679cff]/20">
                        {filteredLogs.length === 0 ? (
                          <div className="text-[#acabaa]/40 italic text-center py-10">
                            No flight recorder logs match your filters.
                          </div>
                        ) : (
                          ...
                          filteredLogs.slice(0, 200).map((log, i) => {
                            ...
                            return (
                              <div
                                key={i}
                                className="flex items-start gap-2.5 hover:bg-[#191a1a]/30 py-0.5 px-1 rounded transition-colors"
                              >
                                <span className="text-[#acabaa]/40 shrink-0 select-none">
                                  {timeStr}
                                </span>
                                <span
                                  className={`font-bold shrink-0 select-none uppercase ${color}`}
                                >
                                  [{log.severity || 'INFO'}]
                                </span>
                                <span className="text-[#e7e5e4] break-all">
                                  <span className="text-[#acabaa]/80 font-bold shrink-0">
                                    {log.caller || 'None'}
                                  </span>{' '}
                                  &rarr; {log.eventType} {log.reason ? `(${log.reason})` : ''}{' '}
              ...
              <div className="bg-[#191a1a]/40 rounded-2xl border border-[#484848]/10 overflow-hidden">
                <button
                  onClick={() =>
                    setCollapsed((prev) => ({ ...prev, diagnostics: !prev.diagnostics }))
                  }
                  className="w-full flex items-center justify-between p-4 bg-transparent border-none text-left cursor-pointer hover:bg-[#1f2020] transition-colors focus:outline-none"
                >
                  <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`material-symbols-outlined text-[#acabaa] transition-transform duration-250 ${!collapsed.diagnostics ? 'rotate-180' : ''}`}
                  >
                    expand_more
                  </span>
                </button>
                  ...
                  <div className="p-4 pt-0 border-t border-[#484848]/5">
                    ...
                    <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                      {filteredDiagnostics.length === 0 ? (
                        <div className="text-center py-10 text-xs italic text-[#acabaa]/40">
                          No diagnostics matches this search.
                        </div>
                      ) : (
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex items-start gap-3">
                                  <span
                                    className={`material-symbols-outlined text-md shrink-0 mt-0.5 ${color}`}
                                  >
                                    {icon}
                                  </span>
                                  <div>
                                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#acabaa]/60">
                                      {entry.subsystem}
                                    </span>
                                    <h4 className="font-extrabold text-xs text-[#e7e5e4] m-0 mt-0.5">
                                      {entry.summary}
                                    </h4>
                                  </div>
                                </div>
                                <span
                                  className={`material-symbols-outlined text-[#acabaa] text-md transition-transform duration-200 ${expanded ? 'rotate-180' : ''}`}
                                >
                                  expand_more
                                </span>
                              </div>
                              ...
                              {expanded && (
                                <div className="mt-3 pt-3 border-t border-[#484848]/10 text-xs space-y-2 leading-relaxed text-[#acabaa]">
                                  <div>
                                    <strong className="block text-[#e7e5e4] text-[10px] uppercase font-black mb-1">
                                      Technical details:
                                    </strong>
                                    <pre className="font-mono bg-[#000000] p-2 rounded text-[10px] text-[#679cff] overflow-x-auto whitespace-pre-wrap">
                                  ...
                                  <div>
                                    ...
                                    <p className="m-0 text-[#e7e5e4]/90">
                                      {entry.humanExplanation}
                                    </p>
                                  </div>
                                  <div>
                                    <strong className="block text-[#e7e5e4] text-[10px] uppercase font-black mb-1">
              ...
              <div className="bg-[#191a1a]/40 rounded-2xl border border-[#484848]/10 overflow-hidden">
                ...
                >
                  ...
                  <span
                    className={`material-symbols-outlined text-[#acabaa] transition-transform duration-250 ${!collapsed.timeline ? 'rotate-180' : ''}`}
                  >
                    expand_more
                  </span>
                </button>
                        ...
                        return (
                          <div key={step.id} className="relative flex gap-4 items-start">
                            ...
                            <div className="flex-1 min-w-0">
                              <h4 className={`text-xs m-0 leading-tight ${textClass}`}>
                                {step.label}
                              </h4>
                              <p className="text-[10px] text-[#acabaa]/60 m-0 mt-0.5 truncate">
                                {step.desc}
                              </p>
                            </div>
                          </div>
              ...
              <div className="bg-[#191a1a]/40 rounded-2xl border border-[#484848]/10 overflow-hidden">
                ...
                >
                  ...
                  <span
                    className={`material-symbols-outlined text-[#acabaa] transition-transform duration-250 ${!collapsed.report ? 'rotate-180' : ''}`}
                  >
                    expand_more
                  </span>
                </button>
                  ...
                  <div className="p-4 pt-0 border-t border-[#484848]/5">
                    <div className="bg-black/90 p-4 rounded-xl border border-[#484848]/10 mt-3">
                      <pre className="font-mono text-[9.5px] leading-relaxed text-[#acabaa]/90 overflow-x-auto whitespace-pre-wrap selection:bg-[#679cff]/20">
                        ...
                        <div className="text-[9px] text-[#acabaa]/40 italic mt-3">
                          // Report has been truncated. Click Copy Report for full trace data.
                        </div>
                      </pre>
                    </div>
                  </div>
