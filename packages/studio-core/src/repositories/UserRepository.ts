...
import { getFirebaseDb, isFirebaseConfigured, incrementFirestoreListeners, decrementFirestoreListeners, incrementFirestoreWrites, decrementFirestoreWrites, setFirestoreLastError } from '../lib/firebase';
import { authRepository, type AuthUser } from './AuthRepository';
...
export class UserRepository {
  ...
  public readonly ACCOUNT_GRACE_DAYS = 7;
  public readonly ACCOUNT_GRACE_MS = this.ACCOUNT_GRACE_DAYS * 24 * 60 * 60 * 1000;
  
  
  
  private metaRef(uid: string) {
    const db = getFirebaseDb();
    if (!db) return null;
    return doc(db, 'users', uid, 'meta', 'account');
  }
  
  private parseDoc(data: any): AccountDoc | null {
  ...
  public async getAccountDoc(uid: string): Promise<AccountDoc | null> {
    const ref = this.metaRef(uid);
    if (!ref) return null;
    try {
      const snap = await getDoc(ref);
      if (!snap.exists()) return null;
      return this.parseDoc(snap.data());
    } catch (err: any) {
  ...
  public async scheduleAccountDeletion(uid: string): Promise<void> {
    const ref = this.metaRef(uid);
    ...
    try {
      ...
      await setDoc(ref, {
        ...
        scheduledAtMs: Date.now() + this.ACCOUNT_GRACE_MS,
  ...
  public async disableAccount(uid: string): Promise<void> {
    const ref = this.metaRef(uid);
    if (!ref) throw new Error('Firebase not configured');
    try {
      incrementFirestoreWrites();
  ...
  public async enableAccount(uid: string): Promise<void> {
    const ref = this.metaRef(uid);
    if (!ref) throw new Error('Firebase not configured');
    try {
      incrementFirestoreWrites();
  ...
  public async cancelAccountDeletion(uid: string): Promise<void> {
    const ref = this.metaRef(uid);
    if (!ref) throw new Error('Firebase not configured');
    try {
      incrementFirestoreWrites();
   ...
   */
  public async finalizeAccountDeletion(uid: string): Promise<void> {
  ...
  public subscribeAccountStatus(
    ...
    const innerUnsub = onSnapshot(
      ref,
      (snap) => cb(snap.exists() ? this.parseDoc(snap.data()) : null),
      (err) => {
        setFirestoreLastError(err.message || String(err));
        cb(null);
      },
   ...
   */
  public subscribeAccountState(cb: (s: AccountState) => void): () => void {
    ...
    const setupStatusListener = (u: AuthUser) => {
      ...
      unsubStatus = this.subscribeAccountStatus(u.uid, (acc) => {
