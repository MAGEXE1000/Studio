  ...
  while (true) {
    ...
    const results = await Promise.all([
      ...
      fetch('https://studio-30f44.web.app/app-release.json').then(async r => {
        ...
        const valid = data.version === '3.7.23' && data.versionCode === 150 && data.signatures === '900cf259185c81100cda8bb08571fa23552e9789131cf07a8f4056e4d4129206';
      ...
      fetch('https://studio-30f44.web.app/version.json').then(async r => {
        ...
        const data = await r.json();
        const valid = data.platform === 'web' && typeof data.version === 'string' && data.commit === '517aa144';
      ...
      Promise.resolve().then(() => {
        const ghResult = spawnSync('gh', ['release', 'view', 'v3.7.23', '--json', 'assets'], { encoding: 'utf8' });
        if (ghResult.status !== 0) return { name: 'gh-release', ok: false, error: ghResult.stderr };
        const assets = JSON.parse(ghResult.stdout).assets;
        const hasApk = assets.some(a => a.name === 'studio-3.7.23.apk');
        const hasSha = assets.some(a => a.name === 'studio-3.7.23.sha256');
      ...
      fetch('https://studio-30f44.web.app/apk/studio-latest.apk', { method: 'HEAD', redirect: 'manual' }).then(r => {
        ...
        const ok = loc.includes('studio-3.7.23.apk');
