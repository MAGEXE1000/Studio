...
const CHORUS: DrumPlugin = {
  ...
  description:
    '3-voice BBD chorus. Thickens and widens drum sounds. Excellent on snares and cymbals.',
  category: 'modulation',
  params: [
  ...
  createNodes(ctx, p) {
    ...
    wet.gain.value = p.mix * 0.7; // scale down to avoid summing too loud
    input.connect(dry);
    dry.connect(output);
    wet.connect(output);
    ...
    const voiceDelays = [0.007, 0.01, 0.013]; // base delay per voice
...
const FLANGER: DrumPlugin = {
  ...
  createNodes(ctx, p) {
    ...
    fbGain.connect(delay); // feedback loop (valid — delay breaks cycle)
    delay.connect(wet);
    wet.connect(output);
...
const PHASER: DrumPlugin = {
  ...
  description:
    '6 all-pass filters swept by LFO. Creates psychedelic spectral movement. Great on hi-hats and cymbals.',
  category: 'modulation',
  params: [
...
const ECHO: DrumPlugin = {
  ...
  description:
    'Tape-style delay with warm repeats. LPF darkens each echo like an analog tape loop.',
  category: 'delay',
  params: [
  ...
  createNodes(ctx, p) {
    ...
    delay.connect(lpf);
    lpf.connect(fbGain);
    fbGain.connect(delay); // tape-dark feedback
    delay.connect(wet);
    wet.connect(output);
...
const TREMOLO: DrumPlugin = {
  ...
  description:
    'Cyclic volume modulation. Sine=smooth pulse, Square=choppy gate. Classic on snares and overhead cymbals.',
  category: 'modulation',
  params: [
...
const BITCRUSH: DrumPlugin = {
  ...
  description:
    'Reduces bit depth for digital lo-fi crunch. 16-bit=CD quality, 4-bit=extreme grit. Great on kicks and snares.',
  category: 'distortion',
  params: [
...
const RINGMOD: DrumPlugin = {
  ...
  description:
    'Multiplies the signal by a carrier sine wave. Creates metallic, bell-like, alien tones.',
  category: 'distortion',
  params: [
  ...
  createNodes(ctx, p) {
    ...
    const output = ctx.createGain();
    const dry = ctx.createGain();
    dry.gain.value = 0.7 * (1 - p.mix);
    const wet = ctx.createGain();
    wet.gain.value = 0; // carrier drives gain
...
const VINYL: DrumPlugin = {
  ...
  description:
    'Analog vinyl character: harmonic drive + 10 kHz air peak. Adds vintage warmth and presence.',
  category: 'distortion',
  params: [
